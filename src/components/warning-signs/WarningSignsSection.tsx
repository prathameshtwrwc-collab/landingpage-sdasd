...
const warningImages: Record<string, string> = {
  sign1: "/assets/section9/Loud-Habitual.png",
  sign2: "/assets/section9/Pauses-in-Breathing.png",
  sign3: "/assets/section9/Waking-up.png",
  sign4: "/assets/section9/Excessive-Daytime.png",
  sign5: "/assets/section9/Falling-Asleep.png",
  sign6: "/assets/section9/Persistent.png",
  sign7: "/assets/section9/Unusual-Movements,.png",
  sign8: "/assets/section9/Significant-impact.png",
};

export default function WarningSignsSection() {
  const { open: openConsult } = useConsult();
  const t = useTranslations("warningSigns");
  return (
    <section
      id="warning-signs"
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={{ hidden: { opacity: 0, y: 25 }, visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } } }} className="relative z-[1] mx-auto max-w-[1120px] px-[20px] max-[389px]:px-[16px] md:px-[32px] lg:px-[48px] pt-[36px] md:pt-[40px] lg:pt-[46px] pb-[38px] md:pb-[44px] lg:pb-[48px] min-w-0">
        ...
        >
          <motion.div variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.05 } } }} className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-[28px_14px] md:gap-[28px_14px] lg:gap-[30px_22px] items-start min-w-0">
            {warningKeys.map((key) => {
              return (
                <motion.div key={key} variants={{ hidden: { opacity: 0, y: 12 }, visible: { opacity: 1, y: 0, transition: { duration: 0.3, ease: "easeOut" } } }} className="flex flex-col items-center text-center warning-sign-item">
                  <div
