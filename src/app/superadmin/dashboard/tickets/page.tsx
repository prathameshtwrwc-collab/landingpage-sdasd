...
import { Ticket, Search, ChevronLeft, ChevronRight, Trash2, CheckCircle, Clock, ArrowRightLeft, Loader2, Eye, X } from "lucide-react";
  ...
  return (
    <DashboardShell title="Support Tickets">
      <div className="max-w-6xl mx-auto">
        <div className="rounded-[16px] p-[20px] md:p-[24px] mb-[16px]" style={{ background: "#FFFFFF", boxShadow: "0 1px 3px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.04)" }}>
          <div className="flex items-center justify-between mb-[16px] flex-wrap gap-[12px]">
            <h2 className="text-[18px] md:text-[20px] font-bold flex items-center gap-[8px]" style={{ fontFamily: "Poppins, sans-serif", fontWeight: 700, color: "#171717" }}>
              <Ticket size={22} style={{ color: "#35319B" }} />
              All Support Tickets
            </h2>
          ...
          {loading ? (
            ...
            <p className="text-center text-[13px] py-[40px]" style={{ color: "#AAA", fontFamily: "Poppins, sans-serif" }}>No tickets found</p>
          ) : (
              ...
              <table className="w-full" style={{ borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#F8F9FF" }}>
                    ...
                    <th className="px-[14px] py-[10px] text-[10px] font-semibold uppercase text-left" style={{ color: "#888" }}>From</th>
                  ...
                  {tickets.map((ticket) => {
                    ...
                    return (
                      <tr key={ticket.id} style={{ borderBottom: "1px solid #F0F0F0" }}>
                        ...
                        <td className="px-[14px] py-[10px] text-[11px] max-w-[300px] truncate" style={{ color: "#555" }}>{ticket.description}</td>
                        <td className="px-[14px] py-[10px] text-[11px]" style={{ color: "#555" }}>
                          <div className="flex items-center gap-[6px]">
                            <span className="inline-flex items-center justify-center w-[20px] h-[20px] rounded-full text-[9px] font-bold text-white" style={{ background: ticket.raised_by_role === "admin" ? "#35319B" : ticket.raised_by_role === "superadmin" ? "#7B68AE" : "#EE8300" }}>
                              {ticket.raised_by_role === "admin" ? "A" : ticket.raised_by_role === "superadmin" ? "S" : "M"}
                            </span>
                            <button
                              type="button"
                              onClick={() => setSenderTicket(ticket)}
                              className="flex items-center gap-[4px] bg-transparent border-none cursor-pointer text-[11px] font-medium underline underline-offset-2"
                              style={{ color: "#35319B", fontFamily: "Poppins, sans-serif", padding: 0 }}
                            >
                              {ticket.sender_name || (ticket.raised_by_role === "member" ? "Member" : ticket.raised_by_role === "admin" ? "Admin" : "Superadmin")}
                              <Eye size={12} />
                            </button>
                          </div>
                        </td>
