...
export async function getMemberDashboard(memberId: string) {
  ...
  if (resultIds.length > 0) {
    const { data: chronoResults } = await supabase
      ...
      .in("id", resultIds);
    if (chronoResults) {
      const resultMap = new Map(chronoResults.map((cr) => [cr.id, cr]));
      reportsEnriched.forEach((r) => {
        if (r.result_id && resultMap.has(r.result_id)) {
          const cr = resultMap.get(r.result_id)!;
          r.chronotype = cr.chronotype;
          r.totalScore = cr.total_score;
          r.larkScore = cr.lark_score;
          r.eagleScore = cr.eagle_score;
          r.owlScore = cr.owl_score;
        }
      });
    }
  }
...
export async function getPublicAssessmentResult(assessmentId: string) {
  const supabase = await createClient();
  ...
  if (assErr || !assessment) return null;
